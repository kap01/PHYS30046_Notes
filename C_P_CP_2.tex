...
\centering
  ...
  \coordinate (r) at (2.9,0);
    ...
    % incoming (left) — set yshift magnitude to control the angle
    \vertex [left=1.2cm of a, yshift=11mm] (i1) {\(e^{-}\)};
    \vertex [left=1.2cm of a, yshift=-11mm] (i2) {\(e^{+}\)};

    % outgoing (right)
    \vertex [right=1.2cm of b, yshift=11mm] (f1) {\(\mu^{-}\)};
    \vertex [right=1.2cm of b, yshift=-11mm] (f2) {\(\mu^{+}\)};

    \diagram*{
      (i1) -- [fermion] (a) -- [photon, edge label=\(\gamma\)] (b) -- [fermion] (f1),
      (i2) -- [anti fermion] (a),
      (b) -- [anti fermion] (f2),
    };
    
  \node [below right=0pt of l] {\(\sqrt{a_{EM}}\)};
  \node [below left=0pt  of r] {\(\sqrt{a_{EM}}\)};
  \end{feynman}
\end{tikzpicture}
\end{center}

However, the same process can proceed via the exchange of two $W$ bosons as
...
\begin{feynman}
  ...
  \coordinate (tr) at (3.2,2.2);

  % external legs
  \node at (-2,0)   (ein)  {\(e^{-}\)};
  \node at (-2,2.2) (epin) {\(e^{+}\)};
  \node at (5.2,0)  (mout) {\(\mu^{-}\)};
  \node at (5.2,2.2)(mpout){\(\mu^{+}\)};

  % draw edges manually with TikZ-Feynman styles
  \draw[fermion]       (ein.east) -- (bl);
  \draw[anti fermion]  (epin.east) -- (tl);
  \draw[fermion]       (br) -- (mout.west);
  \draw[anti fermion]  (tr) -- (mpout.west);

  \draw[fermion]       (bl) -- node[left] {\(\nu_e\)}  (tl);
  \draw[fermion]       (br) -- node[right]{\(\nu_\mu\)}(tr);

  \draw[boson]         (bl) -- node[below]{\(W^{-}\)} (br);
  \draw[boson]         (tl) -- node[above]{\(W^{+}\)} (tr);

  % coupling labels at vertices
  \node [below=2pt of bl] {\(g_W\)};
  \node [above=2pt of tl] {\(g_W\)};
  \node [below=2pt of br]{\(g_W\)};
  \node [above=2pt of tr]{\(g_W\)};
\end{feynman}
\end{tikzpicture}
\end{center}
...
\begin{feynman}
  ...
  \coordinate (bl) at (0,0);
  \coordinate (tl) at (0,2.2);
  \coordinate (br) at (2.2,0);
  \coordinate (tr) at (2.2,2.2);

  % external legs
  \node at (-2,0)   (ein)  {\(e^{-}\)};
  \node at (-2,2.2) (epin) {\(e^{+}\)};
  \node at (4.2,0)  (mout) {\(\mu^{-}\)};
  \node at (4.2,2.2)(mpout){\(\mu^{+}\)};
  ...
  \draw[fermion]       (bl) -- node[left] {\(e^-\)}  (tl);
  \draw[fermion]       (br) -- node[right]{\(\mu^-\)}(tr);

  \draw[boson]         (bl) -- node[below]{\(Z^0\)} (br);
  \draw[boson]         (tl) -- node[above]{\(\gamma\)} (tr);

  % coupling labels at vertices
  \node [below=2pt of bl] {\(g_Z\)};
  \node [above=2pt of tl] {\(\sqrt{\alpha_{EM}}\)};
  \node [below=2pt of br]{\(g_Z\)};
  \node [above=2pt of tr]{\(\sqrt{\alpha_{EM}}\)};

   %--- Define vertices manually
  \coordinate (bl2) at (7.2,0);
  \coordinate (tl2) at (7.2,2.2);
  \coordinate (br2) at (9.4,0);
  \coordinate (tr2) at (9.4,2.2);

  % external legs
  \node at (5.2,0)   (ein)  {\(e^{-}\)};
  \node at (5.2,2.2) (epin) {\(e^{+}\)};
  \node at (11.2,0)  (mout) {\(\mu^{-}\)};
  \node at (11.2,2.2)(mpout){\(\mu^{+}\)};

  % draw edges manually with TikZ-Feynman styles
  \draw[fermion]       (ein.east) -- (bl2);
  \draw[anti fermion]  (epin.east) -- (tl2);
  \draw[fermion]       (br2) -- (mout.west);
  \draw[anti fermion]  (tr2) -- (mpout.west);

  \draw[fermion]       (bl2) -- node[left] {\(e^-\)}  (tl2);
  \draw[fermion]       (br2) -- node[right]{\(\mu^-\)}(tr2);

  \draw[boson]         (bl2) -- node[below]{\(Z^{0}\)} (br2);
  \draw[boson]         (tl2) -- node[above]{\(Z^{0}\)} (tr2);

  % coupling labels at vertices
  \node [below=2pt of bl2] {\(g_Z\)};
  \node [above=2pt of tl2] {\(g_Z\)};
  \node [below=2pt of br2]{\(g_Z\)};
  \node [above=2pt of tr2]{\(g_Z\)}; 
\end{feynman}
\end{tikzpicture}
\end{center}

By relating: the couplings $g_Z$, $g_W$ and $\sqrt{\alpha_{EM}}\,(e)$; the mass of the $W$ and $Z^0$ bosons; as well as the behaviour of processes mediated by the $Z^0$ boson under parity, the sum of the three diagrams shown below can be tamed to give a finite cross-section. 
...
The coupling strengths are related by
...
\begin{feynman}
  ...
  % external legs
  \node at (-2,0)   (ein)  {\(q_{r,g,b}\)};
  \node at (-2,2.2) (epin) {\(\bar{q}_{\overline{r,g,b}}\)};
  \node at (4.2,0)  (mout) {\(q'_{r,g,b}\)};
  \node at (4.2,2.2)(mpout){\(\bar{q}'_{\overline{r,g,b}}\)};
  ...
  \draw[fermion]       (bl) -- node[left] {\(q_{r,g,b}\)}  (tl);
  \draw[fermion]       (br) -- node[right]{\(q'_{r,g,b}\)}(tr);

  \draw[boson]         (bl) -- node[below]{\(Z^0\)} (br);
  \draw[boson]         (tl) -- node[above]{\(\gamma\)} (tr);

  % coupling labels at vertices
  \node [below=2pt of bl] {\(g_{Z}^{q}\)};
  \node [above=2pt of tl] {\(\sqrt{\alpha_{EM}^{q}}\)};
  \node [below=2pt of br]{\(g_{Z}^{q'}\)};
  \node [above=2pt of tr]{\(\sqrt{\alpha_{EM}^{q'}}\)};
  ...
  % external legs
  \node at (5.2,0)   (ein)  {\(q_{r,g,b}\)};
  \node at (5.2,2.2) (epin) {\(\bar{q}_{r,g,b}\)};
  \node at (11.2,0)  (mout) {\(q'_{r,g,b}\)};
  \node at (11.2,2.2)(mpout){\(\bar{q'}_{\overline{r,g,b}}\)};
  ...
  \draw[fermion]       (bl2) -- node[left] {\(q_{r,g,b}\)}  (tl2);
  \draw[fermion]       (br2) -- node[right]{\(q'_{r,g,b}\)}(tr2);

  \draw[boson]         (bl2) -- node[below]{\(Z^{0}\)} (br2);
  \draw[boson]         (tl2) -- node[above]{\(Z^{0}\)} (tr2);

  % coupling labels at vertices
  \node [below=2pt of bl2] {\(g_{Z}^{q}\)};
  \node [above=2pt of tl2] {\(g_{Z}^{q}\)};
  \node [below=2pt of br2]{\(g_{Z}^{q'}\)};
  \node [above=2pt of tr2]{\(g_{Z}^{q'}\)}; 
\end{feynman}
\end{tikzpicture}
...
\begin{feynman}
  ...
  \draw[fermion]       (bl) -- node[left] {\(q''_{r,g,b}\)}  (tl);
  \draw[fermion]       (br) -- node[right]{\(q'''_{r,g,b}\)}(tr);
  ...
  % coupling labels at vertices
  \node [below=2pt of bl] {\(g_{W}^{qq'}\)};
  \node [above=2pt of tl] {\(g_{W}^{qq'}\)};
  \node [below=2pt of br]{\(g_{W}^{q''q'''}\)};
  \node [above=2pt of tr]{\(g_{W}^{q''q'''}\)};
\end{feynman}
\end{tikzpicture}
\end{center}
...
\subsection{The parity of $Z^0$ interactions}
